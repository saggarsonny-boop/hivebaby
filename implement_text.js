...
console.log("Scaffolding HiveText (Wave 1 Micro-Drone)...");

write('apps/hive-text/app/page.tsx', `
"use client";
    ...
    <main className="min-h-screen bg-[#F9FAFB] text-[#111827] flex flex-col font-sans">
       ...
       <nav className="w-full p-6 flex justify-between items-center z-20 border-b border-gray-200 bg-white shadow-sm">
         ...
         <div className="flex gap-2">
            ...
            <button onClick={() => setView('broadcast')} className="text-sm font-semibold px-4 py-2 text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">Broadcast</button>
            <button onClick={() => setView('quiet_hours')} className="text-sm font-semibold px-4 py-2 hover:bg-gray-100 rounded-lg transition-colors">Quiet Hours</button>
         </div>
      </nav>
      ...
      {view === 'home' && (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 animate-fade-in">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl text-left">
              <div onClick={() => setView('broadcast')} className="cursor-pointer bg-white border border-gray-200 p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
              ...
              <div onClick={() => setView('quiet_hours')} className="cursor-pointer bg-white border border-gray-200 p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                 <div className="text-3xl mb-4">🌙</div>
      ...
      {view === 'broadcast' && (
        <div className="flex-1 flex flex-col items-center p-8 bg-gray-50 animate-fade-in">
           <div className="w-full max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
              <h2 className="text-2xl font-bold mb-6">New Broadcast</h2>
...
`);
write('apps/hive-text/app/layout.tsx', `
import "./globals.css";
...
console.log("HiveText scaffolded successfully.");
