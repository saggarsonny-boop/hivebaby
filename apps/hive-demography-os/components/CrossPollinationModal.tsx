  ...
  return (
    <div className="fixed bottom-4 right-4 max-w-sm bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl p-5 z-50 animate-in slide-in-from-bottom-5">
      ...
      <a 
        ...
        onClick={(e) => {
          try {
            const engineId = window.location.hostname.split('.')[0] || 'hive-demography-os';
            fetch('https://creator-console.vercel.app/api/telemetry', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                engine_id: engineId,
                session_id: window.localStorage.getItem('hive_session_id') || 'unknown',
                event_type: 'cross_pollination_click',
