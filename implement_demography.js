...
fs.writeFileSync(path.join(__dirname, 'apps/hive-demography-os/app/page.tsx'), pageContent.trim(), 'utf8');

let layout = fs.readFileSync(path.join(__dirname, 'apps/hive-demography-os/app/layout.tsx'), 'utf8');
layout = layout.replace(/Secret Box/g, 'Demography OS');
layout = layout.replace(/Share a secret. Receive a secret. Feel less alone./g, 'Real-time civilizational infrastructure metrics.');
layout = layout.replace(/bg-black text-white/g, 'bg-white text-black');
fs.writeFileSync(path.join(__dirname, 'apps/hive-demography-os/app/layout.tsx'), layout, 'utf8');

console.log('Demography OS V0 UI implemented.');
