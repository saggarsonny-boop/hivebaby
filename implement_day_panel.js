...
fs.writeFileSync(path.join(__dirname, 'apps/hive-day-panel/app/page.tsx'), pageContent.trim(), 'utf8');

let layout = fs.readFileSync(path.join(__dirname, 'apps/hive-day-panel/app/layout.tsx'), 'utf8');
layout = layout.replace(/Secret Box/g, 'Hive Day Panel');
layout = layout.replace(/Share a secret. Receive a secret. Feel less alone./g, 'Clock. Weather. Calendar. Clarity.');
layout = layout.replace(/bg-black text-white/g, 'bg-[#FDFCF8] text-[#1A1A1A]');
fs.writeFileSync(path.join(__dirname, 'apps/hive-day-panel/app/layout.tsx'), layout, 'utf8');

console.log('Day Panel V0 UI implemented.');
